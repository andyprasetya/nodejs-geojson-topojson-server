...
handlers._billInquiry.post = function(data,callback){
  ...
  if(billctx == '04'){
    _cpoolH2H.query("SELECT idx, dataidx, idbilling, npwpd, th_pajak, context, DATE_FORMAT(dt_payment, '%Y-%m-%d %H:%i:%s') AS dt_payment, DATE_FORMAT(d_payment, '%Y-%m-%d') AS d_payment, nama, alamat, taxobjectidx, taxlocation, tagihan, denda, (tagihan + denda) AS totaltagihan, trx_amount, flag_paid FROM h2h_data WHERE idbilling = ? AND dataidx = ?", [billidx, dataidx], (queryerror, queryresult, fields) => {
      if(queryerror){
